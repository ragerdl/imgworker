 ...
 * ========================
 */
DecoderThread::DecoderThread(PyObject* pyList, Matrix& target, int start_img, int end_img, int img_size, int inner_size, bool test)
: Thread(true), _pyList(pyList), _target(&target), _start_img(start_img), _end_img(end_img),
  _img_size(img_size), _inner_size(inner_size), _test(test), _decodeTarget(0), _decodeTargetSize(0) {
...
void* DecoderThread::run() {
    ...
    assert(_target->getNumRows() == PyList_GET_SIZE(_pyList));
